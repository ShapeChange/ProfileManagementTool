...
function matchesFilter(details, filter) {
    ...
    //console.log('FILTERABLE', filterableDescriptors, filterableTaggedValues);

    return details.name.toLowerCase().indexOf(filter) > -1
        || (details.descriptors && filterableDescriptors.some(key => details.descriptors[key] && details.descriptors[key].toLowerCase().indexOf(filter) > -1))
        || (details.taggedValues && filterableTaggedValues.some(key => details.taggedValues[key] && details.taggedValues[key].toLowerCase().indexOf(filter) > -1))
